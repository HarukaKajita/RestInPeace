...
float3 rand3D(float3 vec){
    ...
    /*
    float rand1 = rand(dot(vec,float3(127.1, 311.7, 264.7)));
    float rand2 = rand(dot(vec,float3(269.5, 183.3, 336.2)));
    float rand3 = rand(dot(vec,float3(301.7, 231.1, 142.6)));
    */
...
float3 valNoise3D(float3 pos){
    float x = valNoise(pos);
    float y = valNoise(float3(pos.y, pos.z, pos.x));
    float z = valNoise(float3(pos.z, pos.x, pos.y));
    return float3(x,y,z);
}
...
//1次元Cellualr Noiseがうまくいかない

float3 cNoise(float3 pos){
    float3 i_o = floor(pos);
    float3 retPos;
...
float3 fbm3D(float3 pos){
    float gain = 0.5;
    float freqIncrease = 3.0;
    float octaves = 3;
    ...
    float maxValue = 0;
    float3 ret = 0.0;//return
    
    for(int i = 0; i < octaves; i++){
        //任意のノイズを使う
        ret += pNoise3D(pos * fre) * amp;
        fre *= freqIncrease;
        maxValue += amp;
        amp *= gain;
    }
